...
8. 打开ET.sln，**编译整个ET.sln**，注意要翻墙，否则可能nuget包下载不下来，导致编译出错(翻墙后如果还有报错解决不了可以尝试先用VS打开ET.sln编译一次后再回到Rider重新编译一次),如果编译错误，提示无效的C#11版本，则是因为msbuilde编译器版本问题，需要msbuilder 17.4,然后在Rider里面选择17.14，dotnet sdk9.0，编译即可通过。
